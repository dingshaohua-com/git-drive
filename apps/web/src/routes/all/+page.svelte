<script lang="ts">
  ...
  let loadingFiles = $derived($query.isLoading);
                ...
                <div class="border-b border-gray-200 p-4">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                      {#if currentPath}
                        <button onclick={goBack} class="px-3 py-2 text-sm bg-gray-100 text-gray-600 rounded hover:bg-gray-200 disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors"> ← 返回 </button>
                      {/if}
                      ...
                      <div class="space-y-1">
                        {#each files as item (item.path)}
                          <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            <div class="flex items-center space-x-3 flex-1 cursor-pointer min-w-0" onclick={() => navigateToDirectory(item.name)}>
