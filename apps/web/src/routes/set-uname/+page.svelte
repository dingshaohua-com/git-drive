...
<div class="min-h-screen flex items-center justify-center bg-gray-50">
	<div class="bg-white rounded-lg shadow-md p-8 w-full max-w-md">
		...
		<form onsubmit={handleSetAccount} class="space-y-4">
			...
			<!-- 个人主页预览 -->
			<div class="p-3 bg-blue-50 rounded-lg border border-blue-200 h-16 flex flex-col justify-center">
				{#if usernameValue}
					<div class="text-sm text-blue-600 mb-1">您的个人主页将为:</div>
					<div class="text-sm font-mono text-blue-800 break-all">
						{typeof window !== 'undefined' ? window.location.origin : ''}/{usernameValue}
					</div>
				{:else}
			...
			<button
				type="submit"
				disabled={loading}
				class="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white font-medium py-3 px-4 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
			>
				{#if loading}
