...
// 使用PBKDF2对密码进行哈希处理
export const toHash = (content: string, saltParam?: string): { hash: string; salt: string } => {
  ...
  const hash = crypto.pbkdf2Sync(content, saltValue, iterations, keyLength, digest).toString('hex');
  return {
    hash,
    salt: saltValue,
  };
